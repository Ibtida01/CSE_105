...
string show1,show2;
int mem[1001][1001];
int c_del,c_ins,c_repl;//c--> cost, del--> deletion , ins --> insertion, repl--> replace, c_del=cost for deletion (with respect to 1st string)
int f(int l1,int l2)
{
    for(int i=0;i<=l2;i++)
        mem[0][i]=i;
    ...
    for(int i=1;i<=l1;i++)
    {
        for(int j=1;j<=l2;j++)
        {
            if(s1[i-1]==s2[j-1])
                mem[i][j]=mem[i-1][j-1];
            else mem[i][j]=min(min(c_del+mem[i][j-1],c_ins+mem[i-1][j]),c_repl+mem[i-1][j-1]);
        }
    }
    return mem[l1][l2];
}
...
void solve()
{
    cout<<"cost for deletion, insertion and replacement :"<<endl;
    cin>>c_del>>c_ins>>c_repl;// int c_del,c_ins,c_repl are delclared globally where c--> cost, del--> deletion , ins --> insertion, repl--> replace, c_del=cost for deletion (with respect to 1st string)
